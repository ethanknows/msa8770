...
caret::confusionMatrix(gbm.pred,testData$Class)   
table(gbm.pred,testData$Class)
#Draw the ROC curve 
...
plotROC(abs(as.numeric(testData$Class)-2), gbm.probs$PS)
AUROC(abs(as.numeric(testData$Class)-2), gbm.probs$PS)
ks_stat(abs(as.numeric(testData$Class)-2), gbm.probs$PS,T)
